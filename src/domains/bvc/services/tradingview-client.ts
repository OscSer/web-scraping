...
export class TradingViewClient {
  ...
  constructor(logger: FastifyBaseLogger) {
    this.tradingViewCache = createCache<number>(TRADINGVIEW_CACHE_TTL_MS, logger);
  }

  async getPriceByTicker(ticker: string): Promise<TradingViewTickerResult | null> {
    ...
    try {
      const price = await this.tradingViewCache.getOrFetch(cacheKey, async () => {
