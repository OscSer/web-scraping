...
export const infoRoutes: FastifyPluginAsync<InfoRoutesOptions> = async (fastify, opts) => {
  const { gameInfoService } = opts;

  fastify.get<{ Querystring: InfoQueryString }>("/info", async (request, reply) => {
    ...
    if (!appId) {
      await sendError(reply, 400, "INVALID_URL", "Invalid Steam URL or App ID");
      return;
    }
